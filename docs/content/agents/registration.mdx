...
- **Benefits**:Don't we want a two part registry?  agent0 is only live on testnet and we plan for our game to go live on mainnet at some point an dit may conflict if the agent0 is still on testnet and we want to use it.  

The thought would be to register the game to agent0 on ethereum testnet and then register all users, agents, actors, game data, reputation and more on base sepolia(for now and later base mainnet)
